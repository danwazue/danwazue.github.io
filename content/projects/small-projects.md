...
<p class="project-body">Road testing exposed the system's main weakness: in a busy environment, everything triggers an alert. The constant alarms made it impossible to distinguish a real threat from background noise — a textbook false-positive problem. The hardware worked. The alert logic needed a lot more refinement to be actually useful. Also, as a bonus, MIT funded a new e-scooter in the process.</p>

---

<h2 class="project-body-heading">Donkey Car — Autonomous Racing</h2>

<p class="project-body">As part of MIT's 16.633 course, I built and trained a <strong>Donkey Car</strong> — an open-source autonomous RC car platform that uses a Raspberry Pi, a forward-facing camera, and a neural network trained on my own driving data. The process: manually drive the car around a track to collect image and control input pairs, train a behavioral cloning model on that data, then load it back onto the car and let it drive itself.</p>
...
<p class="project-body">Last semester, one of the elders at my church asked if I could design and 3D print replacement keys for the paper towel dispensers throughout the building. I jumped in right away — modeled a prototype and printed it the same day. We tested it on the first try. He slid the key into the slot, gave it a twist… and snap — the shaft sheared off and got stuck inside the lock.</p>

<p class="project-body">So it was back to the drawing board. A few iterations later, I landed on a design that could actually handle the shear load. That key now lives in every bathroom in the building. The timing was hard to miss — I was covering torsion in <strong>2.001</strong> at the exact same moment. What I was doing in lecture suddenly had a real-world application I could feel in my hands.</p>

<div class="img-grid cols-2">
  <div class="img-block">
    <img src="/images/church-key/failed-prototype.jpg" alt="Failed key prototype with sheared shaft" class="project-cover-img"/>
    <p class="img-caption">First prototype — shaft sheared on the first attempt</p>
  </div>
  <div class="img-block">
    <img src="/images/church-key/final-key.jpg" alt="Final printed key installed" class="project-cover-img"/>
    <p class="img-caption">Final design, now installed in every bathroom</p>
  </div>
</div>

<p class="project-body">As a Christian, I believe God gives each of us talents to be used for His glory — and in that moment, it felt like I was walking that out. A small design, a cracked key, a few failed prints. But it all came full circle.</p>